...
export const fetchTrendingMovies = async () => {
  ...
  if (!response.ok) throw new Error(`Failed to fetch trending movies. Status: ${response.status}`);
...
export const fetchDirectorMovies = async (directorId: string) => {
  ...
  if (!response.ok) throw new Error(`Failed to fetch director movies. Status: ${response.status}`);
  return await response.json();
}
...
export const fetchTitleDetails = async (tmdbId: string, mediaType: string) => {
  try {
    ...
    if (!response.ok) throw new Error(`Failed to fetch title details. Status: ${response.status}`);
    return await response.json();
  } catch (error) {
